...
if "!ONEAPI_AVAILABLE!"=="1" if "!HAS_MINGW!"=="1" (
    ...
    if "!BUILD_CHOICE!"=="1" (
        ...
        echo Invalid selection. Using default (Intel oneAPI).
        set "BUILD_PRESET=ninja-intel"
        set "USE_ONEAPI=1"
    )
) else if "!ONEAPI_AVAILABLE!"=="1" (
    rem Only Intel oneAPI available
    echo Using Intel oneAPI + Ninja configuration.
    set "BUILD_PRESET=ninja-intel"
    set "USE_ONEAPI=1"
) else if "!HAS_MINGW!"=="1" (
    rem Only MinGW available
    echo Intel oneAPI or Ninja not available. Using MinGW-gcc configuration.
    set "BUILD_PRESET=mingw-gcc"
    set "USE_ONEAPI=0"
) else (
