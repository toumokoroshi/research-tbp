...
try:
    ...
    data = pd.read_csv(
        ...
        names=["Time", "SALI", "x", "y", "z", "px", "py", "pz"],
    )
...
# 必要な列を数値型に変換
numeric_columns = ["Time", "SALI", "x", "y", "z", "px", "py", "pz"]
...
def get_slice(z_target):
    """指定したz値のスライスを取得"""
    return data[
        (data["z"] >= z_target - tolerance) & (data["z"] <= z_target + tolerance)
    ]
...
slider = Slider(ax_slider, "Z", z_min, z_max, valinit=z_initial, valstep=tolerance)
