...
rem   WITH_ONEAPI=1 : Install Intel oneAPI in unattended mode
rem ============================================
...
if exist "C:\Program Files (x86)\Intel\oneAPI\setvars.bat" (
    ...
    if "!UNATTENDED!"=="1" if "!WITH_ONEAPI!"=="0" (
        rem In unattended mode without --with-oneapi, skip installation
        echo     [SKIP] Skipping Intel oneAPI installation in unattended mode.
        echo            Use --with-oneapi to install, or install manually later.
        set "HAS_ONEAPI=0"
    ) else (
        rem Interactive mode OR unattended with --with-oneapi
        if "!WITH_ONEAPI!"=="1" (
            set "INSTALL_ONEAPI=Y"
            echo     [AUTO] Installing Intel oneAPI (--with-oneapi specified)
        ) else (
            ...
            if errorlevel 1 (
                ...
                if "!VCVARS_FOUND!"=="1" (
                    ...
                    if "!HAS_WINSDK!"=="1" (
                        ...
                        echo          [WARNING] Windows SDK not detected yet. May need restart.
                        set "MISSING_REQUIRED=1"
                    )
                ) else (
