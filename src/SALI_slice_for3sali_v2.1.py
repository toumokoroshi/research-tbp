...
def load_file():
    ...
    try:
        ...
        data = pd.read_csv(file_path, sep="\s+", header=None, skiprows=data_start_index, names=[
            "mesh_num", "time", "x", "y", "z", "jacobi constant", "SALI1", "SALI2", "SALI3"
        ])

        numeric_columns = ["x", "y", "z",
                           "jacobi constant", "SALI1", "SALI2", "SALI3"]
...
def create_plot(column):
    ...
    # カスタマイズオプションの適用
    marker_size = int(marker_size_var.get())
    colormap = colormap_var.get()

    sc = ax.scatter(slice_data["x"], slice_data["y"], c=slice_data[column],
                    cmap=colormap, vmin=sali_min, vmax=sali_max, s=marker_size)
    cbar = plt.colorbar(sc, label=column)
    ax.set_xlabel("X(au)")
    ax.set_ylabel("Y(au)")
    title = ax.set_title(
        f"C_j = {slice_data['jacobi constant'].values[0]:.6f}")
    ...
    def update(val):
        ...
        if not slice_data.empty:
            ...
            sc.set_array(slice_data[column].values)
...
dropdown = ctk.CTkOptionMenu(app, variable=column_var, values=[
                             "SALI1", "SALI2", "SALI3"], command=on_column_change)
