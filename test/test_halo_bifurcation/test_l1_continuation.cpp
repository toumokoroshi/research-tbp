/**
 * @file test_l1_continuation.cpp
 * @brief L1 Lyapunov軌道族継続と分岐点検出のテスト
 * @details 実際のL1 Lyapunov軌道族を継続し、ピッチフォーク分岐を検出する
 */
...
int main() {
  ...
  std::cout << "\n========================================" << std::endl;
  std::cout << "  L1 Lyapunov Family Continuation" << std::endl;
  ...
  config.initial_step_size = 0.00001;   // L1用に非常に小さく開始
  config.min_step_size = 1e-9;          // 最小値も小さく
  config.max_step_size = 0.005;
  ...
  config.max_integration_time = 10.0;
  ...
  // 初期振幅（線形近似が有効な小さい値）
  config.initial_amplitude = 0.0001;
  ...
  // L1 Lyapunov軌道族の継続
  // ============================================
  std::cout << "\n[Starting L1 Lyapunov Family Continuation]" << std::endl;
  
  OrbitContinuator<double> continuator(kMuSunEarth, config);
  
  // 推定周期を表示（デバッグ用）
  auto generator = std::make_unique<lyapunov::LyapunovOrbitGenerator<double>>(LagrangePoint::L1, kMuSunEarth);
  ...
  try {
    family = continuator.ContinueLyapunovFamily(LagrangePoint::L1);  // ★ L1に変更
  ...
  std::string output_dir = "data/l1_lyapunov_continuation";  // ★ L1用ディレクトリ
  fs::create_directories(output_dir);

  // 軌道族をCSVに出力
  std::string orbit_file = output_dir + "/l1_lyapunov_family.csv";
  std::ofstream ofs(orbit_file);
  ofs << "# L1 Lyapunov Family\n";
