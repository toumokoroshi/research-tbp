...
//clang-format off
CRTBP::CRTBP(double init_x, double init_y, double init_z, double init_vx,
             double init_vy, double init_vz, double mu, double timestep)
    : init_posi_{init_x, init_y, init_z}, posi_{init_x, init_y, init_z},
      init_v_{init_vx, init_vy, init_vz}, v_{init_vx, init_vy, init_vz},
      init_q_{init_x, init_y, init_z},
      init_p_{init_vx - init_y, init_vy + init_x, init_vz},
      q_{init_x, init_y, init_z}, mu_(3.003e-6), timestep_(timestep) {}
...
CRTBP::CRTBP(std::array<double, 6> init_state, double mu, double timestep)
    : init_posi_{init_state[0], init_state[1], init_state[2]},
      ...
      init_p_{init_state[3] - init_state[0], init_state[4] + init_state[1],
              init_state[5]},
      q_{init_state[0], init_state[1], init_state[2]},
      p_{init_state[3] - init_state[0], init_state[4] + init_state[1],
         init_state[5]},
      mu_(3.003e-6), timestep_(timestep) {}
//clang-format on
...
double CRTBP::calc_r2() const {
  return std::sqrt((posi_[0] - 1. + mu_) * (posi_[0] - 1. + mu_) +
                   posi_[1] * posi_[1] + posi_[2] * posi_[2]);
}
...
void CRTBP::RK4_step_noncanonical() {
  ...
  double r1_3 = std::pow(((posi_[0] + mu_) * (posi_[0] + mu_) +
                          posi_[1] * posi_[1] + posi_[2] * posi_[2]),
                         3. / 2.);
  double r2_3 = std::pow(((posi_[0] - 1. + mu_) * (posi_[0] - 1. + mu_) +
                          posi_[1] * posi_[1] + posi_[2] * posi_[2]),
                         3. / 2.);
  //clang-format off
  k1[3] = 2 * v_[1] + posi_[0] - (1 - mu_) * (posi_[0] + mu_) / r1_3 -
          mu_ * (posi_[0] - 1 + mu_) / r2_3;
  k1[4] = -2 * v_[0] + posi_[1] - (1 - mu_) * posi_[1] / r1_3 -
          mu_ * posi_[1] / r2_3;
  ...
  r1_3 = std::pow(
      ((y1[0] + mu_) * (y1[0] + mu_) + y1[1] * y1[1] + y1[2] * y1[2]), 3. / 2);
  r2_3 = std::pow(
      ((y1[0] - 1 + mu_) * (y1[0] - 1 + mu_) + y1[1] * y1[1] + y1[2] * y1[2]),
      3. / 2);

  k2[3] = 2 * k2[1] + y1[0] - (1 - mu_) * (y1[0] + mu_) / r1_3 -
          mu_ * (y1[0] - 1 + mu_) / r2_3;
  ...
  r1_3 = std::pow(
      ((y2[0] + mu_) * (y2[0] + mu_) + y2[1] * y2[1] + y2[2] * y2[2]), 3. / 2);
  r2_3 = std::pow(
      ((y2[0] - 1 + mu_) * (y2[0] - 1 + mu_) + y2[1] * y2[1] + y2[2] * y2[2]),
      3. / 2);

  k3[3] = 2 * k3[1] + y2[0] - (1 - mu_) * (y2[0] + mu_) / r1_3 -
          mu_ * (y2[0] - 1 + mu_) / r2_3;
  ...
  r1_3 = std::pow(
      ((y3[0] + mu_) * (y3[0] + mu_) + y3[1] * y3[1] + y3[2] * y3[2]), 3. / 2);
  r2_3 = std::pow(
      ((y3[0] - 1 + mu_) * (y3[0] - 1 + mu_) + y3[1] * y3[1] + y3[2] * y3[2]),
      3. / 2);

  k4[3] = 2 * k4[1] + y3[0] - (1 - mu_) * (y3[0] + mu_) / r1_3 -
          mu_ * (y3[0] - 1 + mu_) / r2_3;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    v_[i] +=
        timestep_ / 6 * (k1[i + 3] + 2 * k2[i + 3] + 2 * k3[i + 3] + k4[i + 3]);
  }
}
void CRTBP::RK4_step_canonical() {
  ...
  auto calc_r1_ = [&](const std::array<double, 3> &q_temp) {
    return std::sqrt((q_temp[0] + mu_) * (q_temp[0] + mu_) +
                     q_temp[1] * q_temp[1] + q_temp[2] * q_temp[2]);
  };
...
CRTBP::trajectory_propagation(double endtime) {
