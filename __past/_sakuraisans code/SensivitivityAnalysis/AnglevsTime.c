...
int main() {
    C = 2.999649;
    outputfile = fopen("outputfile_angletime.d", "w");
    if (outputfile == NULL) {
        printf("  can not open write file.");
    }
    for(angle = -M_PI/4.0 ; angle <= M_PI/4.0 ; angle += 0.001){
        ...
        vx = vx1 + delta_Vx * cos(angle) + delta_Vy * sin(angle);
        vy = vy1 - delta_Vx * sin(angle) + delta_Vy * cos(angle);
    ...
    fprintf(myfile, "set xlabel 'Angle, -'\n");
    ...
    fprintf(myfile, "plot 'outputfile_angletime.d' w l\n");
    return 0;
}