...
int main() {
    ...
    for(alfa=0.7 ; alfa <= 1.3 ; alfa += 0.01){
        for(angle = -M_PI/4.0 ; angle <= M_PI/4.0 ; angle += 0.01){
            ...
            vx = vx1 + delta_Vx * cos(angle) * alfa + delta_Vy * sin(angle) * alfa;
            vy = vy1 - delta_Vx * sin(angle) * alfa + delta_Vy * cos(angle) * alfa;
