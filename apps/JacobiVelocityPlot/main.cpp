...
std::vector<DataPoint> GenerateSampleData(int32_t n_samples, double mu, double perturb_range,
  ...
  // 初期速度vyも対数スケールでサンプリング（ΔCの分布を均一化するため）
  const double log_vy_min = std::log10(0.001);  // 最小vy = 0.001
  const double log_vy_max = std::log10(1.0);    // 最大vy = 1.0
  std::uniform_real_distribution<double> dist_log_vy(log_vy_min, log_vy_max);
  // 速度摂動も対数スケールでサンプリング
  const double log_perturb_min = std::log10(perturb_range * 1e-4);  // 最小値 = 最大値の1e-4
  const double log_perturb_max = std::log10(perturb_range);
  std::uniform_real_distribution<double> dist_log_perturb(log_perturb_min, log_perturb_max);
  ...
  for (int32_t i = 0; i < n_samples; ++i) {
    ...
    // Generate random initial velocity (対数スケールでサンプリング)
    const double vx = 0.0;
    const double vy = std::pow(10.0, dist_log_vy(rng));  // 対数スケール
    const double vz = 0.0;
    ...
    // Apply log-uniform velocity perturbation (対数スケールで均一)
    // dvy は負方向の摂動として適用（元の速度を減少させる）
    const double perturb_magnitude = std::pow(10.0, dist_log_perturb(rng));
    const double dvx = 0.0;
    const double dvy = -perturb_magnitude;  // 負方向に摂動
...
bool GenerateGnuplotScript(const std::string& script_path, const std::string& csv_path,
  ...
  ofs << R"(# Gnuplot script for log-log visualization of Jacobi Integral Change vs Velocity Change
...
# Log-log scale for uniform distribution
set logscale x
set logscale y
set format x "10^{%L}"
set format y "10^{%L}"
...
set title 'CR3BP: Velocity Change vs Jacobi Integral Change (Log-Log)\n(Color: x position, y=z=0)' font 'Arial,14'
...
# Plot - using absolute value for delta_c since log scale requires positive values
plot ')" << csv_path
      << R"(' using 1:(abs($2)):4 with points pt 7 ps 0.5 palette notitle
...
bool GenerateInteractiveScript(const std::string& script_path, const std::string& csv_path) {
  ...
  ofs << R"(# Interactive Gnuplot script for log-log visualization
# Use mouse to zoom and explore the plot

set datafile separator ','
set terminal wxt size 1200,1000 enhanced persist title 'Jacobi Velocity Log-Log Plot'
...
set title 'CR3BP: Velocity Change vs Jacobi Integral Change (Log-Log)\n(Color: Jacobi Integral C)\n(Use mouse to zoom)' font 'Arial,14'

# Color palette for Jacobi integral
set palette defined (2.0 "dark-blue", 2.5 "blue", 3.0 "green", 3.5 "yellow", 4.0 "red")
set cblabel 'Jacobi Integral C [-]'
set cbrange [2.0:3.5]

# Grid and mouse settings
set mouse
set grid
set key top left

# Initial plot - using absolute value for delta_c
plot ')" << csv_path
      << R"(' using 1:(abs($2)):3 with points pt 7 ps 0.5 palette notitle

print "Interactive mode enabled!"
print "  - Left mouse drag:  Select zoom region"
print "  - Right mouse:      Unzoom"
print "  - 'a' key:          Autoscale"
