...
int main() {
  ...
  // $$EOE
  Vector3d<double> target_pos_SSB_perigee{7.304196237871223E-01, 6.098274565642665E-01,
                                          2.646650323945715E-01};
  Vector3d<double> target_vel_SSB_perigee{-1.224114608078465E-02, 1.204646234284942E-02,
                                          5.261882690463215E-03};
  Vector3d<double> target_pos_SSB_apogee{5.257984240277522E-01, 7.577655799250410E-01,
                                         3.283858264463194E-01};
  Vector3d<double> target_vel_SSB_apogee{-1.433600204809335E-02, 8.127139041478151E-03,
  ...
  // $$EOE
  Vector3d<double> p2_pos_SSB_perigee{7.284388574310334E-01, 6.086682022418486E-01,
                                      2.639921635473907E-01};
  Vector3d<double> p2_vel_SSB_perigee{-1.187458565021005E-02, 1.158622746231203E-02,
                                      5.021849812165025E-03};
  Vector3d<double> p2_pos_SSB_apogee{5.273914666583941E-01, 7.596884055466107E-01,
                                     3.294606499098446E-01};
  Vector3d<double> p2_vel_SSB_apogee{-1.478997985135310E-02, 8.422006642370750E-03,
                                     3.650959645440828E-03};

  std::array<Vector3d<double>, 2> target_state_SSB_perigee{target_pos_SSB_perigee,
                                                           target_vel_SSB_perigee};
  std::array<Vector3d<double>, 2> target_state_SSB_apogee{target_pos_SSB_apogee,
                                                          target_vel_SSB_apogee};
  std::array<Vector3d<double>, 2> p2_state_SSB_perigee{p2_pos_SSB_perigee, p2_vel_SSB_perigee};
  std::array<Vector3d<double>, 2> p2_state_SSB_apogee{p2_pos_SSB_apogee, p2_vel_SSB_apogee};

  std::array<Vector3d<double>, 2> target_state_SEBR_apogee =
      ConvertInertial2RotatingV2<Vector3d<double>, double, 2>(target_state_SSB_apogee,
                                                              p2_state_SSB_apogee, astro_params);
  std::array<Vector3d<double>, 2> target_state_SEBR_perigee =
      ConvertInertial2RotatingV2<Vector3d<double>, double, 2>(target_state_SSB_perigee,
                                                              p2_state_SSB_perigee, astro_params);
  std::cout << std::setprecision(15);
  std::cout << "target pos SEBR at apogee = " << target_state_SEBR_apogee[0].x() << " "
            << target_state_SEBR_apogee[0].y() << " " << target_state_SEBR_apogee[0].z()
            << std::endl;
  std::cout << "target vel SEBR at apogee = " << target_state_SEBR_apogee[1].x() << " "
            << target_state_SEBR_apogee[1].y() << " " << target_state_SEBR_apogee[1].z()
            << std::endl;
  std::cout << "target pos SEBR at perigee = " << target_state_SEBR_perigee[0].x() << " "
            << target_state_SEBR_perigee[0].y() << " " << target_state_SEBR_perigee[0].z()
            << std::endl;
  std::cout << "target vel SEBR at perigee = " << target_state_SEBR_perigee[1].x() << " "
            << target_state_SEBR_perigee[1].y() << " " << target_state_SEBR_perigee[1].z()
            << std::endl;
  ...
  //    kmに直してプリント
  std::cout << "target pos SEBR at apogee (km) = " << target_state_SEBR_apogee[0].x() * kAU / 1000.0
            << " " << target_state_SEBR_apogee[0].y() * kAU / 1000.0 << " "
            << target_state_SEBR_apogee[0].z() * kAU / 1000.0 << " " << std::endl;
