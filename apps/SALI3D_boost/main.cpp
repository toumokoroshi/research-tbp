...
int main() {
  ...
  std::cout << "<>        2. Scan Earth's vecinity (multi-config file)"
            << std::endl;
  std::cout << "<>        3. Scan the optional trajectory (single config file)"
            << std::endl;
  std::cout << "<>        else. Exit" << std::endl;
  std::cout << "<>      Input number : ";
  std::cin >> mode2;
  ...
  while (ifs) {
    ...
    for (const auto &point : meshPoints) {
      ...
      // non-biased velocity
      double v_abs = calc_v_abs(point, JACOBI_INTEGRAL, MU);
      double vx, vy, vz = 0;
    ...
    std::cout << "<>        Simulation for " << configfilename << " finished"
              << std::endl;
    configdata_num++;
    configfilename = "configdata/3D_crtbp_SALI/3DSALIconfig_" +
